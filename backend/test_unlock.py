...
def fixture_pdf(encrypted=True):
    ...
    result = doc.tobytes(encryption=pymupdf.PDF_ENCRYPT_AES_256 if encrypted else pymupdf.PDF_ENCRYPT_NONE,
...
def request(pdf, password):
    ...
    return client.post('/api/unlock', content=len(secret).to_bytes(4, 'big') + secret + pdf,
...
def test_malformed_body():
    assert client.post('/api/unlock', content=b'bad', headers={'Content-Type': 'application/octet-stream'}).status_code == 422
