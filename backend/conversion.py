...
async def convert(request: Request):
    ...
    if protection not in {"locked", "unlocked"} or (
        protection == "locked" and target != "pdf"
    ):
        raise HTTPException(
            422, "Password protection is available for PDF output only."
        )
    if not (
        source == "pdf"
        and target in {"docx", "xlsx", "pptx", "md"}
        or source in OFFICE | IMAGES | {"txt", "md", "markdown", "pdf"}
        and target == "pdf"
    ):
    ...
    async with gate:
        with tempfile.TemporaryDirectory(prefix="pdf-convert-") as folder:
            ...
            with input_file.open("wb") as stream:
                async for chunk in request.stream():
                    if not prefix_done:
                        ...
                        if not 1 <= length <= 40:
                            raise HTTPException(
                                422, "Use a password of 1–40 UTF-8 bytes."
                            )
                        ...
                        try:
                            ...
                            raise HTTPException(
                                422, "Enter a valid, non-blank password."
                            )
            ...
            if target != "pdf":
                command = [
                    sys.executable,
                    str(Path(__file__).with_name("convert_document.py")),
                    str(input_file),
                    target,
                    str(output),
                ]
                ...
                command = [
                    sys.executable,
                    str(Path(__file__).with_name("create_pdf.py")),
                    str(input_file),
                    str(output),
                ]
            try:
                ...
                raise HTTPException(
                    503, "The conversion engine is not installed on this server."
                )
            try:
                ...
                if isinstance(error, asyncio.CancelledError):
                    raise
                raise HTTPException(
                    408, "Conversion timed out. Try a smaller document."
                )
            ...
            if process.returncode or not output.exists():
                raise HTTPException(
                    422,
                    "Unable to convert. Check the file is valid and unlocked, with at most 200 pages. Text must be UTF-8 or UTF-16; images must be at most 24 megapixels. Scanned PDFs need OCR before Word/Excel conversion.",
                )
            if output.stat().st_size > 80 * 1024 * 1024:
                raise HTTPException(
                    413, "Converted document exceeds 80 MB. Try fewer pages."
                )
            return Response(
                output.read_bytes(),
                media_type=TYPES[target],
                headers={
