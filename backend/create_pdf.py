...
def create_pdf(source, output, password=""):
    ...
    if kind in OFFICE:
        ...
        subprocess.run(
            [
                "libreoffice",
                "-env:UserInstallation=" + (source.parent / "profile").as_uri(),
                "--headless",
                "--convert-to",
                "pdf",
                "--outdir",
        ...
        content = "".join(
            "<p>" + html.escape(line.expandtabs(4)) + "</p>"
        ...
        with Image.open(source) as image, fitz.open() as doc:
            ...
            for index in range(frames):
                ...
                page.insert_image(
                    page.rect + (36, 36, -36, -36), stream=data.getvalue()
                )
    ...
    with fitz.open(intermediate) as doc:
        if not doc.is_pdf or doc.needs_pass or not 0 < doc.page_count <= 200:
            raise ValueError(
                "Use an unlocked document with 1–200 pages. Unlock protected PDFs first."
            )
