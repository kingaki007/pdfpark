...
def isolated_database():
    ...
    try:
        ...
        with psycopg.connect(original, autocommit=True) as db:
            db.execute(sql.SQL('DROP DATABASE {} WITH (FORCE)').format(sql.Identifier(test_name)))
...
def clients(monkeypatch):
    ...
    with TestClient(app) as first, TestClient(app) as second:
        for client in (first, second):
            ...
            result = client.post('/api/auth/register', json={'email': email, 'password': 'correct-horse-battery'})
...
def test_auth_and_csrf(clients):
    ...
    assert first.post('/api/auth/logout', headers={'X-PDF-Studio': ''}).status_code == 403
    assert first.post('/api/auth/logout', headers={'Sec-Fetch-Site': 'cross-site'}).status_code == 403
    ...
    assert first.get('/api/jobs', headers={'Cookie': 'pdf_session=' + cookie}).status_code == 401
    assert first.post('/api/auth/login', json={'email': accounts[0], 'password': 'wrong-password'}).status_code == 401
    login = first.post('/api/auth/login', json={'email': accounts[0].upper(), 'password': 'correct-horse-battery'})
...
def test_jobs_isolation_and_failure(clients):
    ...
    assert first.post('/api/jobs', content=b'bad', headers={'Content-Type': 'text/plain'}).status_code == 415
    assert first.post('/api/jobs', content=b'bad', headers={'Content-Type': 'application/pdf'}).status_code == 422
    result = first.post('/api/jobs?name=sample', content=b'%PDF-invalid', headers={'Content-Type': 'application/pdf'})
...
def test_real_ocr_download(clients):
    first, second, _ = clients
    result = first.post('/api/jobs?name=search-test', content=pdf_fixture(), headers={'Content-Type': 'application/pdf'})
    assert result.status_code == 202
    job_id = result.json()['id']
    assert run_once()
