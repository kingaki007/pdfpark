...
async def security(request: Request, call_next):
    ...
    if request.method in {"POST", "DELETE", "PUT", "PATCH"}:
        if (
            request.headers.get("x-pdf-studio") != "1"
...
def user(request: Request):
    with connect() as db:
        row = db.execute(
            "SELECT u.id, u.email, u.is_root FROM users u JOIN sessions s ON s.user_id=u.id "
...
def auth_limit(request):
    ...
    with connect() as db:
        ...
        row = db.execute(
            "INSERT INTO auth_limits VALUES (%s, 1, now()+interval '15 minutes') "
...
def start_session(db, user_id, response, request):
    ...
    db.execute(
        "DELETE FROM sessions WHERE expires_at<now() OR token=%s",
        (digest(request.cookies.get(COOKIE, "")),),
    )
    db.execute(
        "INSERT INTO sessions VALUES (%s,%s,now()+interval '7 days')",
    ...
    try:
        with connect() as db:
            db.execute(
                ...
                (user_id, email, hash_password(data.password)),
            )
...
def login(data: Credentials, request: Request, response: Response):
    ...
    with connect() as db:
        row = db.execute(
            "SELECT * FROM users WHERE email=%s", (data.email.strip().lower(),)
        ).fetchone()
...
async def create_job(request: Request, current=Depends(user)):
    ...
    try:
        with connect() as db:
            if not db.execute(
                "SELECT id FROM users WHERE id=%s FOR UPDATE", (current["id"],)
            ).fetchone():
                raise HTTPException(401, "Account no longer exists.")
            count = db.execute(
                "SELECT count(*) AS n FROM jobs WHERE user_id=%s", (current["id"],)
            ).fetchone()["n"]
            if count >= 10:
                raise HTTPException(
                    409,
                    "Your library is full (10 documents). Delete a job before uploading.",
                )
            storage.write(reference, data)
            db.execute(
                "INSERT INTO jobs(id,user_id,name,input_path) VALUES (%s,%s,%s,%s)",
...
def jobs(current=Depends(user)):
    with connect() as db:
        return db.execute(
            "SELECT id,name,status,created_at,error FROM jobs WHERE user_id=%s "
...
def owned(db, job_id, current):
    row = db.execute(
        "SELECT id,name,status,input_path,output_path FROM jobs WHERE id=%s AND user_id=%s FOR UPDATE",
        (job_id, current["id"]),
    ).fetchone()
    if not row:
...
def download(job_id: uuid.UUID, current=Depends(user)):
    with connect() as db:
        ...
        try:
            data = (
                storage.path(row["output_path"]).read_bytes()
                if row["output_path"]
                else None
            )
...
def delete(job_id: uuid.UUID, current=Depends(user)):
    with connect() as db:
        ...
        db.execute(
            "DELETE FROM jobs WHERE id=%s AND user_id=%s", (job_id, current["id"])
        )
