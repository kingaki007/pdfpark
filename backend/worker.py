...
def process(data):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if not any(
            page.extract_text().strip() for page in PdfReader(io.BytesIO(output)).pages
        ):
...
def run_once():
    with connect() as db:
        stale = db.execute(
            "SELECT id,input_path FROM jobs WHERE status='processing' "
            "AND started_at<now()-interval '15 minutes' FOR UPDATE"
        ).fetchall()
        for job in stale:
            storage.remove(job["input_path"])
            db.execute(
                "UPDATE jobs SET status='failed',input_path=NULL,error='Worker interrupted. Upload again.' "
                "WHERE id=%s",
                (job["id"],),
            )
        row = db.execute(
            "SELECT id,input_path FROM jobs WHERE status='queued' ORDER BY created_at "
    ...
    try:
        ...
        with connect() as db:
            ...
            current = db.execute(
                "SELECT status FROM jobs WHERE id=%s FOR UPDATE", (row["id"],)
            ).fetchone()
            if current and current["status"] == "processing":
                storage.write(reference, output)
                db.execute(
                    "UPDATE jobs SET status='completed',output_path=%s,input_path=NULL WHERE id=%s",
                    (reference, row["id"]),
                )
    except Exception:
        with connect() as db:
            ...
            if current and current["status"] == "processing":
                ...
                db.execute(
                    "UPDATE jobs SET status='failed',input_path=NULL,error=%s WHERE id=%s",
                    (
                        "OCR could not finish. Use a readable, unencrypted PDF of up to 200 pages; "
                        "large files may exceed the 10-minute limit.",
                        row["id"],
                    ),
                )
    finally:
