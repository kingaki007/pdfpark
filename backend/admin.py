...
def seed_root():
    ...
    with connect() as db:
        ...
        if db.execute("SELECT id FROM users WHERE email=%s", (email,)).fetchone():
            raise RuntimeError(
                "Root email belongs to an existing account; choose ROOT_ADMIN_EMAIL"
            )
        db.execute(
            "INSERT INTO users(id,email,password,is_root) VALUES (%s,%s,%s,true)",
...
def list_users(current=Depends(root_user)):
    with connect() as db:
        return db.execute(
            "SELECT u.id,u.email,u.is_root,count(j.id) AS document_count "
...
def list_files(current=Depends(root_user)):
    with connect() as db:
        return db.execute(
            "SELECT j.id,j.user_id,u.email,j.name,j.status,j.created_at,j.error, "
            "(j.input_path IS NOT NULL) AS has_input, "
            "(j.output_path IS NOT NULL) AS has_output "
            "FROM jobs j JOIN users u ON u.id=j.user_id ORDER BY j.created_at DESC"
        ).fetchall()
...
def download_file(job_id: uuid.UUID, kind: str, current=Depends(root_user)):
    ...
    with connect() as db:
        row = db.execute(
            "SELECT input_path,output_path FROM jobs WHERE id=%s FOR UPDATE", (job_id,)
        ).fetchone()
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="{job_id}-{kind}.pdf"'},
    )
...
def delete_user(user_id: uuid.UUID, current=Depends(root_user)):
    with connect() as db:
        row = db.execute(
            "SELECT is_root FROM users WHERE id=%s FOR UPDATE", (user_id,)
        ).fetchone()
        if not row:
        ...
        rows = db.execute(
            "SELECT input_path,output_path FROM jobs WHERE user_id=%s ORDER BY id FOR UPDATE",
            (user_id,),
        ).fetchall()
