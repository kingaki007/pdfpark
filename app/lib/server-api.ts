export type OcrJob = { id: string; name: string; status: 'queued' | 'processing' | 'completed' | 'failed'; created_at: string; error: string | null };
...
export async function api(path: string, options: RequestInit = {}): Promise<Response> {
  ...
  if (!response.ok) {
    ...
    throw new Error(typeof body?.detail === 'string' ? body.detail :
      response.status === 413 ? 'The edited PDF exceeds the 40 MB server limit.' :
