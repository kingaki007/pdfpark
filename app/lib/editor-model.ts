...
export type Mark = {
  id: string;
  kind: "text" | "image" | "rectangle" | "ellipse";
  x: number;
  y: number;
  width: number;
  height: number;
...
export function toPagePoint(
  x: number,
  y: number,
  page: Pick<EditorPage, "width" | "height" | "rotation">,
) {
...
export function rotationTransform(page: Pick<EditorPage, "width" | "height" | "rotation">) {
  switch (page.rotation) {
    case 90:
      return `translate(${page.height} 0) rotate(90)`;
    case 180:
      return `translate(${page.width} ${page.height}) rotate(180)`;
    case 270:
      return `translate(0 ${page.width}) rotate(270)`;
    default:
      return "";
  }
}
export function fitMark(mark: Mark, page: EditorPage): Mark {
  ...
  return {
    ...
    width,
    height,
    x: Math.max(0, Math.min(page.width - width, mark.x)),
    y: Math.max(0, Math.min(page.height - height, mark.y)),
  };
}
