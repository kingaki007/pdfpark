...
export function groupTextBlocks(items: ExtractedItem[], page: EditorPage): Mark[] {
  ...
  for (const item of [...items].sort((a, b) => a.baseline - b.baseline || a.x - b.x)) {
    const row = rows.findLast(
      (r) =>
        Math.abs(r.y - item.baseline) < Math.min(r.size, item.style.fontSize) * 0.25 &&
        item.x >= r.x &&
        item.x - r.right < r.size * 3,
    );
    if (row) {
      row.items.push(item);
      row.right = Math.max(row.right, item.x + item.width);
      row.size = Math.max(row.size, item.style.fontSize);
    } else
      rows.push({
        x: item.x,
        y: item.baseline,
  ...
  for (const row of rows) {
    const block = blocks.findLast((b) => {
      const last = b.at(-1)!;
      return (
        row.y > last.y + last.size * 0.5 &&
        row.y - last.y <= last.size * 1.8 &&
        Math.abs(row.x - last.x) <= 24 &&
        Math.abs(row.size - last.size) <= 2
      );
    });
  ...
  return blocks.map((rows, index) => {
    ...
    const bottom = Math.min(page.height, rows.at(-1)!.y + rows.at(-1)!.size * 0.3);
    const lines: TextLine[] = rows.map((row, i) => {
      ...
      for (const item of row.items.sort((a, b) => a.x - b.x)) {
        ...
        if (gap > item.style.fontSize * 0.15)
          runs.push({
            text: " ".repeat(Math.max(1, Math.round(gap / (item.style.fontSize * 0.3)))),
            style: item.style,
          });
      ...
      return {
        runs,
        indent: row.x - x,
        leading: rows[i + 1] ? rows[i + 1].y - row.y : row.size * 1.2,
      };
    });
    ...
    return {
      ...
      height: bottom - y,
      lines,
      text: lines.map((l) => l.runs.map((r) => r.text).join("")).join("\n"),
      fontSize: rows[0].size,
      color: lines[0].runs[0].style.color,
      stroke: 0,
      background: "#ffffff",
    };
  });
