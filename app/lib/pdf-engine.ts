...
pdfjs.GlobalWorkerOptions.workerSrc =
    workerUrl + (workerUrl.includes("?") ? "&" : "?") + "worker-mime=js-v2";
...
export async function importPdf(bytes: Uint8Array): Promise<EditorPage[]> {
    if (bytes.length > MAX_FILE_SIZE)
        throw new Error(
            "This file exceeds the 40 MB limit. Split or compress it first.",
        );
    if (!new TextDecoder().decode(bytes.slice(0, 1024)).includes("%PDF-"))
        throw new Error(
            "This is not a supported PDF file. Choose a PDF, PNG, or JPEG.",
        );
    ...
    try {
        ...
        if (editable.getForm().getFields().length)
            throw new Error(
                "Interactive or signed forms are not supported yet. Flatten a copy in your PDF viewer, then import it.",
            );
        document = await pdfjs.getDocument({ data: bytes.slice(), ...options })
            .promise;
        if (document.numPages > MAX_PAGES)
            throw new Error(
                `This PDF exceeds the ${MAX_PAGES}-page limit. Split it into smaller documents.`,
            );
        const sourceId = crypto.randomUUID(),
            pages: EditorPage[] = [];
        for (let i = 0; i < document.numPages; i++) {
            const p = await document.getPage(i + 1),
                view = p.getViewport({ scale: 1 });
            if (view.width > 14400 || view.height > 14400)
                throw new Error(
                    "This PDF contains a page larger than the supported size.",
                );
            pages.push({
        ...
        sources.set(sourceId, { bytes, document });
        return pages;
    } catch (error) {
        await document?.destroy().catch(() => {});
        const message = error instanceof Error ? error.message : String(error);
        if (
            /worker|dynamically imported module|module script|fetch/i.test(
                message,
            )
        )
            throw new Error(
                "The PDF rendering worker could not load. Refresh the app. If this continues, check that the server serves .mjs files as application/javascript.",
            );
        if (/password|encrypt/i.test(message))
            throw new Error(
                "This PDF is encrypted or password-protected. Use Unlock PDF in the header to enter its password and download an unlocked copy, then open that copy.",
            );
        if (/limit|exceeds|not supported|supported size/i.test(message))
            throw error;
        throw new Error(
            "This PDF is damaged or uses an unsupported structure. Try opening and saving a fresh copy in a PDF viewer.",
        );
    }
}
export async function imageData(file: File): Promise<string> {
    if (file.size > MAX_FILE_SIZE)
        throw new Error("Images must be smaller than 40 MB.");
    ...
    if (!png && !jpg)
        throw new Error("Unsupported image. Choose a PNG or JPEG file.");
    ...
    if (img.naturalWidth * img.naturalHeight > 24_000_000)
        throw new Error(
            "This image exceeds 24 megapixels. Resize it before importing.",
        );
    return data;
}
export async function imagesToPdf(files: File[]): Promise<Uint8Array> {
    ...
    for (const file of files) {
        ...
        const image = data.startsWith("data:image/png")
...
export function loadImage(data: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () =>
            reject(
                new Error(
                    "The image could not be decoded. Choose another PNG or JPEG.",
                ),
            );
        image.src = data;
    });
}
export function markTextLines(mark: Mark): string[] {
...
export async function overlayPng(page: EditorPage): Promise<Uint8Array | null> {
    ...
    for (const m of page.marks) {
        if (m.sourceRect) {
            ctx.fillStyle = m.background || "#ffffff";
            const r = m.sourceRect;
        ...
        if (m.kind === "text") {
            ...
            for (const line of getLines(m)) {
                ...
                for (const run of line.runs) {
                    ctx.font = canvasFont(run.style);
                    ctx.fillStyle = run.style.color;
                    const text = expandTabs(run.text);
                    ctx.fillText(text, x, top + lineSize(line));
                    x += ctx.measureText(text).width;
                }
                top += Math.max(line.leading, lineSize(line));
            }
        } else if (m.kind === "image" && m.data)
            ctx.drawImage(await loadImage(m.data), m.x, m.y, m.width, m.height);
            ...
            ctx.ellipse(
                m.x + m.width / 2,
                m.y + m.height / 2,
                Math.max(1, (m.width - m.stroke) / 2),
                Math.max(1, (m.height - m.stroke) / 2),
                0,
                0,
                Math.PI * 2,
            );
    ...
    const blob = await new Promise<Blob>((resolve, reject) =>
        canvas.toBlob(
            (b) =>
                b
                    ? resolve(b)
                    : reject(new Error("Could not render the export.")),
            "image/png",
        ),
    );
...
export async function exportPdf(pages: EditorPage[]) {
    ...
    for (const page of pages) {
        ...
        if (!source)
            throw new Error(
                "The original file is no longer available. Reopen the PDF.",
            );
        ...
        if (page.marks.some((mark) => mark.sourceRect)) {
            ...
            const viewport = pdfPage.getViewport({
                scale: Math.min(2, 4096 / Math.max(page.width, page.height)),
            });
            const canvas = document.createElement("canvas");
            ...
            if (overlay) {
                const url = URL.createObjectURL(
                    new Blob([overlay.slice().buffer as ArrayBuffer], {
                        type: "image/png",
                    }),
                );
                try {
                ...
                canvas.toBlob(
                    (b) =>
                        b
                            ? resolve(b)
                            : reject(
                                  new Error("Could not flatten edited text."),
                              ),
                    "image/png",
                ),
            );
            overlays[page.id] = new Uint8Array(await blob.arrayBuffer());
            canvas.width = canvas.height = 0;
        }
    }
    return new Promise<Uint8Array>((resolve, reject) => {
        const worker = new Worker(
            new URL("./pdf-export.worker.ts", import.meta.url),
            { type: "module" },
        );
        const timeout = setTimeout(() => {
            worker.terminate();
            reject(new Error("Export took too long. Try fewer pages."));
        }, 120_000);
        const finish = () => {
        ...
        worker.onerror = () => {
            finish();
            reject(
                new Error("The export worker stopped. Try a smaller document."),
            );
        };
...
export async function exportJpg(pages: EditorPage[], name: string) {
    ...
    try {
        for (let i = 0; i < doc.numPages; i++) {
            ...
            files[`${name}-${i + 1}.jpg`] = new Uint8Array(
