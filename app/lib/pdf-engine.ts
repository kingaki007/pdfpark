...
export async function importPdf(bytes: Uint8Array): Promise<EditorPage[]> {
  ...
  if (!new TextDecoder().decode(bytes.slice(0, 1024)).includes("%PDF-"))
    throw new Error("This is not a supported PDF file. Choose a PDF, PNG, or JPEG.");
  ...
  try {
    ...
    for (let i = 0; i < document.numPages; i++) {
      ...
      if (view.width > 14400 || view.height > 14400)
        throw new Error("This PDF contains a page larger than the supported size.");
      pages.push({
    ...
    if (/worker|dynamically imported module|module script|fetch/i.test(message))
      throw new Error(
...
export async function imageData(file: File): Promise<string> {
  ...
  if (img.naturalWidth * img.naturalHeight > 24_000_000)
    throw new Error("This image exceeds 24 megapixels. Resize it before importing.");
  return data;
}
...
export function loadImage(data: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error("The image could not be decoded. Choose another PNG or JPEG."));
    image.src = data;
  });
}
...
export async function overlayPng(page: EditorPage): Promise<Uint8Array | null> {
  ...
  const blob = await new Promise<Blob>((resolve, reject) =>
    canvas.toBlob(
      (b) => (b ? resolve(b) : reject(new Error("Could not render the export."))),
      "image/png",
    ),
  );
...
export async function exportPdf(pages: EditorPage[]) {
    ...
    if (page.marks.some((mark) => mark.sourceRect)) {
      ...
      if (overlay) {
        ...
        try {
          canvas
            .getContext("2d")!
            .drawImage(await loadImage(url), 0, 0, canvas.width, canvas.height);
        } finally {
      ...
      const blob = await new Promise<Blob>((resolve, reject) =>
        canvas.toBlob(
          (b) => (b ? resolve(b) : reject(new Error("Could not flatten edited text."))),
          "image/png",
        ),
      );
  ...
  return new Promise<Uint8Array>((resolve, reject) => {
    ...
    worker.onerror = () => {
      finish();
      reject(new Error("The export worker stopped. Try a smaller document."));
    };
    worker.postMessage({ pages, sources: originals, overlays });
  });
}
export async function exportJpg(pages: EditorPage[], name: string, format: "jpg" | "png" = "jpg") {
  ...
  try {
    for (let i = 0; i < doc.numPages; i++) {
      ...
      const blob = await new Promise<Blob>((resolve, reject) =>
        canvas.toBlob(
          (b) => (b ? resolve(b) : reject(new Error(format.toUpperCase() + " export failed."))),
          mime,
          0.92,
        ),
      );
      files[`${name}-${i + 1}.${format}`] = new Uint8Array(await blob.arrayBuffer());
      canvas.width = canvas.height = 0;
    }
  } finally {
    await doc.destroy();
