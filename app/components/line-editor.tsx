...
export function LineEditor({
  ...
  return (
    <fieldset disabled={disabled} className="line-editor">
      ...
      {lines.map((l, i) => (
        <label key={i}>
          ...
          <input
            ...
            style={{
              whiteSpace: "pre",
              fontFamily: l.runs[0].style.fontFamily,
            }}
            onChange={(e) =>
              onChange(
                withLines(
                  mark,
                  lines.map((v, n) => (n === i ? replaceLineText(v, e.target.value) : v)),
                ),
              )
            }
          />
      ...
      {line && style ? (
        <>
          <span className="muted">
            Formatting line {index + 1}. New lines inherit its style and indentation.
          </span>
          <div className="row">
            <Button
              variant="outline"
              onClick={() => {
                onChange(withLines(mark, insertLine(lines, index, false)));
                setActive(index);
              }}
            >
              Insert above
            ...
            <Button
              variant="outline"
              onClick={() => {
                onChange(withLines(mark, insertLine(lines, index)));
                setActive(index + 1);
              }}
            >
              Insert below
          ...
          <div className="property-grid">
            ...
            <label>
              Indent (pt)
              <input
                aria-label="Line indentation"
                type="number"
                min={0}
                max={mark.width - 8}
                value={Math.round(line.indent)}
                onChange={(e) =>
                  onChange(
                    withLines(
                      mark,
                      lines.map((l, i) =>
                        i === index
                          ? {
                              ...
                              indent: Math.max(
                                0,
                                Math.min(mark.width - 8, Number(e.target.value) || 0),
                              ),
                            }
                          : l,
                      ),
      ...
      {mark.sourceRect && (
        <label>
          ...
          <input
            ...
            type="color"
            value={mark.background || "#ffffff"}
            onChange={(e) => onChange({ ...mark, background: e.target.value })}
          />
        </label>
      )}
      <p className="muted">
        Lines reflow within this paragraph. Nearby paragraphs and images stay in place.
      </p>
    </fieldset>
  );
}
