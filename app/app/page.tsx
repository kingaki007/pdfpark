...
export default function Home() {
    ...
    const change = useCallback(
        (next: DocumentState | ((doc: DocumentState) => DocumentState)) =>
            setHistory((h) =>
                commit(h, typeof next === "function" ? next(h.present) : next),
            ),
        [],
    );
    ...
    const applyTextMark = (next: Mark) => {
            ...
            ...getLines(next).map(
                (line) =>
                    ...
                    line.runs.reduce((sum, run) => {
                        ...
                        return (
                            sum +
                            measure.measureText(expandTabs(run.text)).width
                        );
                    }, 0) +
                    2,
            ),
        ...
        if (
            selected &&
            neighbors.some((m) => overlap(next, m) && !overlap(selected, m))
        ) {
            setError(
    ...
    async function run(label: string, action: () => Promise<void>) {
        ...
        try {
            ...
            setError(
                e instanceof Error
                    ? e.message
                    : "The operation failed. Try another file.",
            );
        } finally {
        ...
        await run("Opening files…", async () => {
            ...
            try {
                ...
                if (pages.length + (append ? doc.pages.length : 0) > MAX_PAGES)
                    throw new Error(
                        "A document can contain at most " +
                            MAX_PAGES +
                            " pages.",
                    );
                const next = {
                    name:
                        append && doc.pages.length
                            ? doc.name
    ...
    async function save() {
        await run("Preparing download…", async () => {
            ...
            const name =
                doc.name.replace(/[^\p{L}\p{N}_ -]/gu, "").slice(0, 100) ||
                "document";
            if (format === "pdf") {
                download(
                    await exportPdf(pages),
                    name +
                        (pages.length < doc.pages.length ? "-selected" : "") +
                        ".pdf",
                    "application/pdf",
                );
                ...
                const output = await exportJpg(pages, name);
        ...
        <div className="studio">
            ...
            <div className="workspace">
                    ...
                    {!page ? (
                        <div className="empty-workspace">
                            ...
                            <p>
                                ...
                                <br />
                                and signatures. Editing stays on your device.
                                OCR is optional.
                            </p>
                            <Button
                                size="lg"
                            ...
                            <span className="muted">
                                or drop files here · PDF, PNG, JPG · up to 40 MB
                                each
                            </span>
                            <button
                                className="text-link"
                ...
                <aside className="properties-panel">
                    <div className="panel-heading">
                        <strong>
                            {selected ? "Object properties" : "Document tools"}
                        </strong>
                    </div>
                    {selected ? (
                        ...
                        <div className="properties">
                            <p className="muted">
                                Choose a tool above, then click or drag on the
                                page.
                            </p>
                    ...
                    <div className="privacy-note">
                        ...
                        <div>
                            ...
                            <p>
                                Editing stays in this browser. OCR, unlocking
                                and document conversion upload files only when
                                you request them.
                            </p>
                        </div>
                    </div>
            ...
            <Dialog open={signatureOpen} onOpenChange={setSignatureOpen}>
                <DialogContent className="signature-dialog">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Draw below, or upload a PNG/JPG of your signature.
                            This adds a signature image.
                        </DialogDescription>
                    </DialogHeader>
                    <SignaturePad
                        onSave={(data) => {
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Download all pages or choose a range.
                        </DialogDescription>
                    </DialogHeader>
            ...
            <AlertDialog open={leaveOpen} onOpenChange={setLeaveOpen}>
                <AlertDialogContent>
                    ...
                    <AlertDialogDescription>
                        Any unsaved changes will be lost. Leave and return to
                        home?
                    </AlertDialogDescription>
                    <AlertDialogFooter>
            ...
            >
                <AlertDialogContent>
                    ...
                    <AlertDialogDescription>
                        Your current changes have not been exported. Replacing
                        the document will discard them.
                    </AlertDialogDescription>
                    <AlertDialogFooter>
