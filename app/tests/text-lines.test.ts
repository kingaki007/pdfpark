...
import { getLines, insertLine, lineText, replaceLineText, withLines, type TextLine, type TextStyle } from '../lib/text-lines';
import { groupTextBlocks } from '../lib/text-extraction';
import { commit, undo, redo, type Mark, type EditorPage } from '../lib/editor-model';
const style: TextStyle = { fontFamily: 'Courier New', fontSize: 12, bold: true, italic: false, color: '#123456' };
const line: TextLine = { indent: 24, leading: 18, runs: [{ text: '  First ', style }, { text: 'second', style: { ...style, italic: true, color: '#ff0000' } }] };
const mark: Mark = { id: 'm', kind: 'text', x: 30, y: 40, width: 200, height: 36, fontSize: 12, stroke: 0, color: '#123456', lines: [line, { ...line, runs: [{ text: 'Last', style }] }] };
const page: EditorPage = { id: 'p', sourceId: 's', sourceIndex: 0, width: 600, height: 800, rotation: 0, marks: [mark] };
describe('formatted text lines', () => {
  it('inserts above and below with copied indentation, whitespace, font, color and spacing', () => {
    ...
    expect(below[0]).toBe(original[0]); expect(below[2]).toBe(original[1]);
  ...
  it('shrinks content after removal, keeps the original cover, and supports undo/redo', () => {
    ...
    expect(removed.height).toBe(18); expect(removed.sourceRect).toEqual(original.sourceRect);
    const document = { name: 'test', pages: [{ ...page, marks: [original] }] };
    const history = commit({ past: [], present: document, future: [] }, { ...document, pages: [{ ...page, marks: [removed] }] });
