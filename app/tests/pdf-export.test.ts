...
it("merges, selects, reorders and rotates actual PDF pages without changing originals", async () => {
  ...
  b.addPage([600, 700]).setRotation(degrees(90));
  const sources = { a: await a.save(), b: await b.save() };
  const pages: EditorPage[] = [
    { id: "b0", sourceId: "b", sourceIndex: 0, width: 700, height: 600, rotation: 90, marks: [] },
    { id: "a1", sourceId: "a", sourceIndex: 1, width: 400, height: 500, rotation: 0, marks: [] },
  ];
  const output = await PDFDocument.load(
    await assemblePdf(
      pages,
      (id) => sources[id as keyof typeof sources],
      async () => null,
    ),
  );
  expect(output.getPageCount()).toBe(2);
  expect(output.getPage(0).getSize()).toEqual({ width: 600, height: 700 });
  expect(output.getPage(0).getRotation().angle).toBe(180);
  expect(output.getPage(1).getWidth()).toBe(400);
...
it.each([
  ...
  [180, 210, 320, 200, 300],
  [270, 10, 320, 300, 200],
])("aligns overlays with crop origins and intrinsic rotation %s", (r, x, y, width, height) => {
  expect(overlayPlacement({ x: 10, y: 20, width: 200, height: 300 }, r)).toEqual({
    x,
    y,
    width,
    height,
...
it.each([0, 90, 180, 270])(
  "exports edited original text as a fresh image page at rotation %s",
  async (rotation) => {
    const png = Uint8Array.from(
      Buffer.from(
        "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScLbtAAAAABJRU5ErkJggg==",
      ...
      marks: [
        {
          ...
          fontSize: 12,
          stroke: 0,
          sourceRect: { x: 10, y: 10, width: 80, height: 20 },
        },
      ],
    };
    const output = await PDFDocument.load(
      await assemblePdf(
        [page],
        () => {
          throw new Error("Original content must not be copied");
        },
        async () => png,
      ),
    );
    ...
    ).rejects.toThrow("edited page image is missing");
  },
);
